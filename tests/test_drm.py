...
@pytest.mark.parametrize('module,kwargs,expected', [
    (pbs, {'time' : timedelta(days=2, minutes=6), 'workers' : 4, 'memInGB' : 2.345},
...
@pytest.mark.parametrize('module,kwargs,expected', [
    (slurm, {'time' : timedelta(days=2, minutes=6), 'workers' : 4, 'memInGB' : 2.345},
...
def test_slurm_submit(tmpdirs):
    ...
    assert fp == path(script_dir).joinpath(submit.script_name_join.join(['test', submit.uid]))
...
def test_pbs_submit(tmpdirs):
    ...
    assert re.search('^#PBS -d {0}$'.format(path.getcwd()), script, re.MULTILINE)
