...
class BaseSubmitter(FormatterObject):
    ...
    def submit_job(
        ...
        script_fp = self.scriptDir.joinpath(self.script_name_join.join([script_name, self.uid]))
    ...
    def _add_hold(self, flag_list, **kwargs):
        ...
        '''
        self.__run_format_method(flag_list, self.format_hold, self.__map_name_to_jid(kwargs.get('hold')))

    def _add_logDir(self, flag_list, **kwargs):
        '''
        Defaults to the directory optionally passed at construction
        '''
        self.__run_format_method(flag_list, self.format_logDir, (kwargs.get('logDir') or self.logDir))


    def _add_name(self, flag_list, **kwargs):
        self.__run_format_method(flag_list, self.format_name, kwargs.get('name'))
    ...
    def _add_workDir(self, flag_list, **kwargs):
        ...
        '''
        self.__run_format_method(flag_list, self.format_workDir, (kwargs.get('workDir') or path.getcwd()))
    ...
    def __submit_and_validate(self, script_fp, name=None):
        ...
        if name in self._JOB_NAME_TO_ID:
            message = 'Name {0} already in _JOB_NAME_TO_ID with value {1}'.format(name, self._JOB_NAME_TO_ID.get(name))
